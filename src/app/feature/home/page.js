        ...
        <div className={"flex flex-col w-full h-full bg-none justify-center text-center"}>
            ...
            <div className="relative top-0 w-full h-[80vh] bg-[url(/background.png)] flex justify-center bg-cover">
                ...
                <div className="absolute searchslot w-[60%] h-16 bottom-10 flex items-center pl-6 pr-4 gap-4">
                    ...
                    <input type="text" placeholder = "제품명을 입력하세요" id="search" minLength="0" className="absolute w-[950px] h-full left-20 outline-0
                        text-xl font-medium focus:outline-none focus:ring-0 focus:shadow-none"/>
                    <button className={"absolute bg-[#FADD88] hover:bg-[#FFD24E] rounded-full w-60 h-[80%] right-2 font-semibold text-[#505050] text-lg select-transition tracking-widest"}>
                        SEARCH</button>
                </div>

                <div className={"absolute bg-gray-30 w-[75%] h-8 bottom-8 rounded-2xl"}></div>
            </div>
        </div>
    )
}
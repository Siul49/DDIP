                    ...
                    <button key={item.key} onClick={() => handleClick(item.value)} className="relative w-full h-full p-4
                        ...
                        <div className={"absolute w-full h-[90%]"}>
                            <Image src={`/categoryImg/${item.value}.png`}
                                   fill
                                   className=" w-full h-full bg-[#FFF5D8] hover:bg-[#FFC6B8] rounded-2xl p-4 object-contain select-transition mb-2"
                                   alt="이미지"/>
                        </div>
                        <p className={"absolute w-full h-[15%] bottom-[-15%] z-50 bg-[#FFF5D8] hover:bg-[#FFC6B8] rounded-2xl text-2xl"}>
                            {item.name || '이름 없음'}
                        </p>
                    </button>
                ))}
